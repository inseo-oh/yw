...
func html_cors_settings_from_attr(elem dom.Element, attr_name string) html_cors_settings {
	if attr, ok := elem.AttrWithoutNamespace(attr_name); ok {
		if attr == "" {
...
func html_fetch_priority_from_attr(elem dom.Element, attr_name string) html_fetch_priority {
	if attr, ok := elem.AttrWithoutNamespace(attr_name); ok {
		switch cm.ToAsciiLowercase(attr) {
		case "high":
