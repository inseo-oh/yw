...
func (tk tagToken) equals(other html_token) bool {
	other_tk, ok := other.(*tagToken)
	if !ok {
		return false
	}
	...
	for i := 0; i < len(tk.attrs); i++ {
		...
		if attr.LocalName != other_tk.LocalName {
			return false
		}
...
func (tk tagToken) is_start_tag() bool {
	return !tk.is_end
}
func (tk tagToken) is_end_tag() bool {
	return tk.is_end
}
...
func (t *html_tokenizer) run() {
	...
	attrs_to_remove := []int{}
	curr_tag_token := func() *tagToken {
		tok := curr_tok.(*tagToken)
		return tok
	}
	...
	check_duplicate_attr_name := func() {
		...
		for i, attr := range attrs {
			...
			if curr_attr().LocalName == attr.LocalName {
				attrs_to_remove = append(attrs_to_remove, i)
			}
		}
	}
	emit_token := func(tk html_token) {
		if tag_tk, ok := tk.(*tagToken); ok {
			final_attrs := []dom.AttrData{}
	...
	is_appropriate_end_tag_token := func(tk tagToken) bool {
	...
	for {
		...
		case html_tokenizer_before_attribute_name_state:
			...
			case '=':
				...
				curr_tag_token().attrs = append(curr_tag_token().attrs, dom.AttrData{
					LocalName: string(next_char),
					Value:     "",
				})
				attrs_to_remove = []int{}
