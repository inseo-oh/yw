...
var encoding_label_map = map[string]Type{
...
func GetEncodingFromLabel(label string) (Type, error) {
	label = libcommon.ToAsciiLowercase(strings.TrimFunc(label, func(r rune) bool { return r == ' ' }))
	encoding, ok := encoding_label_map[label]
	if !ok {
...
type handler_result int64 // Positive values are actual result, negative values are special results(see below)
const (
	handler_result_error    = handler_result(-99) // https://encoding.spec.whatwg.org/#error
	handler_result_finished = handler_result(-98) // https://encoding.spec.whatwg.org/#finished
	handler_result_continue = handler_result(-97) // https://encoding.spec.whatwg.org/#continue
)
...
type decoder interface {
	handler(queue *IoQueue, byte_item IoQueueItem) handler_result
}

// https://encoding.spec.whatwg.org/#decode
func Decode(input *IoQueue, fallback_encoding_type Type, output *IoQueue) {
	encoding_type := fallback_encoding_type
	bom_encoding, ok := bom_sniff(*input)
	if ok {
		encoding_type = bom_encoding
		if bom_encoding == Utf8 {
			input.Read(3)
		} else {
			input.Read(2)
		}
	}
	encoding, ok := encodings[encoding_type]
	if !ok {
		panic("unsupported encoding")
	}
	decoder := encoding.make_decoder()
	decode(decoder, input, output, error_mode_replacement)
}

type error_mode uint8

const (
	error_mode_replacement = error_mode(iota)
	error_mode_html
	error_mode_fatal
)

func decode(decoder decoder, input *IoQueue, output *IoQueue, mode error_mode) handler_result {
	for {
		item := input.ReadOne()
		res := decode_item(item, decoder, input, output, mode)
		if res != handler_result_continue {
			return res
		}
	}
}
func decode_item(item IoQueueItem, decoder decoder, input *IoQueue, output *IoQueue, mode error_mode) handler_result {
	if mode == error_mode_html {
		panic("invalid error_mode")
	}
	res := decoder.handler(input, item)
	if res == handler_result_finished {
		output.PushOne(IoQueueItem{EndOfQueue{}})
		return res
	} else if 0 <= res {
		if libcommon.IsSurrogateChar(rune(res)) {
			panic("result cannot contain surrogate char")
		}
		output.PushOne(IoQueueItem{rune(res)})
	} else if res == handler_result_error {
		switch mode {
		case error_mode_replacement:
			output.PushOne(IoQueueItem{rune(0xfffd)})
		case error_mode_html:
			panic("unreachable")
		case error_mode_fatal:
			return res
		}
	}
...
func bom_sniff(queue IoQueue) (Type, bool) {
...
func (q *IoQueue) Read(num int) []IoQueueItem {
	read_items := []IoQueueItem{}
	for range num {
		item := q.ReadOne()
		if !item.IsEndOfQueue() {
			read_items = append(read_items, item)
		}
	}
	return read_items
}
