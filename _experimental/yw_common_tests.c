...
void yw_test_shrink_to_fit(YW_TestingContext *ctx)
{
    int len = 0;
    int cap = 0;
    char *buf = YW_ALLOC(char, 1); /* Doesn't matter what size it is */
...
#define YW_RUN_TEST(_name, _input, ...)                                                                                    \
    ...
    YW_RUN_TEST("Three byte characters", "\xe0\xa4\x80\xed\x9f\xbb\xef\xad\x8f", 0x0900, 0xd7fb, 0xfb4f);
    YW_RUN_TEST("Four byte characters", "\xf0\x90\x91\x90\xf0\x9f\x83\xb5\xf4\x81\x8a\x8f", 0x10450, 0x1f0f5, 0x10128f);
